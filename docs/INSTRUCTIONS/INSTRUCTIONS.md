...
Полученный IFC-файл — это библиотека шаблонов свойств, которую можно импортировать в BonsaiBIM.

![Blender с аддоном Bonsai (BlenderBIM). На правой панели открыт раздел «Property Set Templates» с импортированной библиотекой, сгенерированной IDS2PSET. Выделен PSet «МОГЭ_Информация», внутри которого видны все шаблоны свойств: Наименование (IfcText), Кадастровый номер (IfcText), Тип зоны (IfcText), Вместимость (IfcInteger), МГН (IfcBoolean) и другие. Каждый шаблон содержит тип данных IFC и формат значения (P_SINGLEVALUE, P_ENUMERATEDVALUE). Слева — 3D-вид здания с деревом элементов модели](screenshots/05-usage-in-BBIM.png)

Импортированные шаблоны становятся доступны для выбора на соответствующих элементах.

![Blender + Bonsai: применение PSet к элементу модели. На правой панели «Object Information» выбран элемент IfcSite. Раздел «Property Sets» раскрыт, показан PSet «МОГЭ_Информация» с полями для ввода значений: текстовые поля (Наименование,  Кадастровый номер), числовые поля (Масса = 0.00, Вместимость = 0), чекбоксы (МГН, Временный), переключатели (Возрастная  группа: 1, 2, 3), выпадающий список с вариантами (Вид работ: Выемка, Насыпь, Демонтаж и др.). Шаблон PSet из IFC-библиотеки позволяет заполнять свойства элемента модели](screenshots/06-applying-PSets-in-BBIM.png)
